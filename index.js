...
const getAllFlags = async (countryCodes) => {

    await Promise.all(countryCodes.map(getFlag));

}

const getFlag = async code => {

    return axios.get(`https://www.countryflags.io/${code}/flat/32.png`, {
        responseType: 'stream'
     }).then(response => {
         response.data.pipe(fs.createWriteStream(`flags/ic_${code.toLowerCase()}.png`))
     });
}
